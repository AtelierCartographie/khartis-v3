...
- **ComboBox `selectedId` vs late `items`**: the dropdown (`filteredItems`) IS reactive to `items`, but `selectedItem` is only resolved when `selectedId` _changes_ (`prevSelectedId !== selectedId`), never when `items` arrives _after_ mount. So a ComboBox mounted before its `items` are ready shows an **empty field until it is remounted** (e.g. reopening the surrounding accordion). Fix by forcing a remount with `{#key <signature>}` where the signature is a **stable string** that changes only when the option set genuinely changes (e.g. `Array.from(valueSet).sort().join(...)`) — never key on the array reference itself (recreated each render → permanent remount → blank field). See `join-assisted-section.svelte` (`joinedValueSignature`).
