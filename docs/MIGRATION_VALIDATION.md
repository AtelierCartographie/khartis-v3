...
const detection = await GeoColumnDetector.detectGeoColumns(
  headers,
  data
);
...
if (detection.suggestedPrimaryGeoColumn) {
  ...
  console.log(`Using ${geoColumn} (${detection.suggestedPrimaryGeoColumn.confidence * 100}% confidence)`);
}
```
...
result.fuzzyMatches.forEach(match => {
  console.log(`"${match.original}" → "${match.suggestion}" (${match.similarity * 100}%)`);
});
```
