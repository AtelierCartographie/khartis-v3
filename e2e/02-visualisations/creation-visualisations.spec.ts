...
test.describe('Création de visualisations - 2.B.1', () => {
  test("crée automatiquement une visualisation à l'étape", async ({ page }) => {
    ...
    const vizPanel = page
      .locator('[data-testid="visualization-panel"]')
      .or(page.locator('.visualization-panel, .viz-panel'));
    await expect(vizPanel).toBeVisible({ timeout: 10000 });

    const vizList = page
      .locator('[data-testid="viz-item"]')
      .or(page.locator('.viz-item, .visualization-item'));
  ...
  test('nomme par défaut "Visualisation (1)"', async ({ page }) => {
    ...
    const vizName = page
      .locator('[data-testid="viz-name"]')
      .or(page.locator('.viz-name, .visualization-name'))
      .first();
    const name = await vizName.textContent();
    expect(name).toMatch(/Visualisation.*1|Visualization.*1/);
  });

  test.skip('incrémente le nom des visualisations suivantes', async ({
    page
  }) => {});
  ...
  test('permet de supprimer une visualisation', async ({ page }) => {
    ...
    const deleteBtn = page
      .locator('[data-testid="viz-delete"]')
      .or(
        page.locator(
          'button[aria-label*="supprimer"], button[aria-label*="delete"]'
        )
      )
      .first();
  ...
  test('permet de créer plusieurs visualisations', async ({ page }) => {
    ...
    const addVizBtn = page
      .locator('[data-testid="add-viz"]')
      .or(
        page.locator(
          'button:has-text("Ajouter"), button[aria-label*="ajouter"]'
        )
      )
      .first();
...
test.describe('Suggestions de visualisations - 2.B.2.a', () => {
  test('propose des visualisations selon les données', async ({ page }) => {
    ...
    const suggestions = page
      .locator('[data-testid="viz-suggestion"]')
      .or(page.locator('.suggestion-card, .viz-suggestion'));
  ...
  test('affiche les vignettes des suggestions', async ({ page }) => {
    ...
    const suggestionThumbnails = page
      .locator('[data-testid="suggestion-thumbnail"]')
      .or(page.locator('.suggestion-thumbnail, .suggestion img'));
    if ((await suggestionThumbnails.count()) > 0) {
      await expect(suggestionThumbnails.first()).toBeVisible();
    }
  });

  test.skip('affiche les primitives graphiques utilisées', async ({
    page
  }) => {});
  ...
  test('sélectionne la meilleure suggestion par défaut', async ({ page }) => {
    ...
    const firstSuggestion = page
      .locator('[data-testid="viz-suggestion"]')
      .or(page.locator('.suggestion-card'))
      .first();
  ...
  test.skip('préremplit les paramètres de la suggestion', async ({
    page
  }) => {});
});
...
test.describe('Paramétrage de visualisation - 2.B.2.b', () => {
  test('regroupe les réglages par primitives', async ({ page }) => {
    ...
    const primitiveGroups = page
      .locator('[data-testid="primitive-group"]')
      .or(page.locator('.primitive-settings, .settings-group'));
  ...
  test('affiche/masque une primitive', async ({ page }) => {
    ...
    const visibilityToggle = page
      .locator('[data-testid="primitive-visibility"]')
      .or(page.locator('button[aria-label*="visibility"]'))
      .first();
  ...
  test('règle la taille des éléments', async ({ page }) => {
    ...
    const sizeSlider = page
      .locator('[data-testid="size-slider"]')
      .or(
        page.locator(
          'input[type="range"][aria-label*="taille"], input[type="range"][aria-label*="size"]'
        )
      )
      .first();
  ...
  test("règle l'épaisseur des traits", async ({ page }) => {
    ...
    const strokeSlider = page
      .locator('[data-testid="stroke-width"]')
      .or(
        page.locator(
          'input[type="range"][aria-label*="épaisseur"], input[type="range"][aria-label*="width"]'
        )
      )
      .first();
