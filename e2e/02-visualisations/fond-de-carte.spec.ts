...
test.describe('Personnalisation du fond de carte - 2.B.3', () => {
  test('modifie la couleur de fond des polygones', async ({ page }) => {
    ...
    const mapSettings = page.locator('[data-testid="map-settings"]').or(page.locator('.map-settings, button:has-text("Fond de carte")'));
    if (await mapSettings.count() > 0) {
      await mapSettings.first().click();
      await page.waitForTimeout(500);

      const colorPicker = page.locator('[data-testid="background-color"]').or(page.locator('.color-picker').first());
  ...
  test('modifie la couleur des contours', async ({ page }) => {
    ...
    if (await mapSettings.count() > 0) {
      ...
      const strokeColor = page.locator('[data-testid="stroke-color"]').or(page.locator('button:has-text("Contour")').first());
  ...
  test('règle l\'épaisseur des contours', async ({ page }) => {
    ...
    if (await mapSettings.count() > 0) {
      ...
      const strokeWidth = page.locator('[data-testid="stroke-width"]').or(page.locator('input[type="range"]').first());
...
test.describe('Fond de carte du catalogue - 2.B.3.a', () => {
  test('affiche les couches disponibles', async ({ page }) => {
    ...
    const layerCatalog = page.locator('[data-testid="layer-catalog"]').or(page.locator('.layer-catalog, button:has-text("Catalogue")'));
    if (await layerCatalog.count() > 0) {
      await layerCatalog.first().click();
      await page.waitForTimeout(500);

      const layerItems = page.locator('.layer-item, [data-testid="layer-item"]');
  ...
  test('affiche/masque la couche terre', async ({ page }) => {
    ...
    if (await layerCatalog.count() > 0) {
      ...
      if (await landLayer.count() > 0) {
        const toggle = landLayer.locator('..').locator('input[type="checkbox"], button').first();
        await toggle.click();
        await page.waitForTimeout(500);
      }
    }
  ...
  test.skip('affiche/masque la couche lacs et rivières', async ({ page }) => {});
  ...
  test.skip('personnalise chaque couche individuellement', async ({ page }) => {});
});
