...
test.describe('Outil de recherche - 2.B.4.a', () => {
  ...
  test('recherche une entité sur la carte', async ({ page }) => {
    const searchTool = page.locator('[data-testid="search-tool"]').or(page.locator('.search-tool'));
    await expect(searchTool).toBeVisible({ timeout: 10000 });

    const searchInput = searchTool.locator('input[type="search"], input[type="text"]').first();
    await searchInput.fill('France');
    await page.keyboard.press('Enter');

    await page.waitForTimeout(1000);

    const searchResults = searchTool.locator('.search-result, [data-testid="search-result"]');
  ...
  test('met en lumière l\'élément trouvé', async ({ page }) => {
    ...
    const highlightedElement = page.locator('.highlighted, [data-highlighted="true"]').first();
  ...
  test('affiche le nombre de résultats', async ({ page }) => {
    ...
    const resultCount = searchTool.locator('.result-count, [data-testid="result-count"]').first();
...
test.describe('Gestion des calques - 2.B.4.b', () => {
  ...
  test('crée un calque par visualisation', async ({ page }) => {
    const layersPanel = page.locator('[data-testid="layers-tool"]').or(page.locator('.layers-panel'));
    await expect(layersPanel).toBeVisible({ timeout: 10000 });

    const layerItems = layersPanel.locator('.layer-item, [data-testid="layer-item"]');
  ...
  test('identifie les calques par couleur et icône', async ({ page }) => {
    ...
    const firstLayer = layersPanel.locator('.layer-item, [data-testid="layer-item"]').first();
  ...
  test('affiche/masque un calque', async ({ page }) => {
    ...
    const firstLayer = layersPanel.locator('.layer-item, [data-testid="layer-item"]').first();
    const visibilityToggle = firstLayer.locator('[data-testid="layer-visibility"], .visibility-toggle, button[aria-label*="visibility"]').first();

    await visibilityToggle.click();
    await page.waitForTimeout(500);

    const isHidden = await visibilityToggle.getAttribute('aria-pressed') === 'false' ||
                     await visibilityToggle.getAttribute('data-visible') === 'false';

    await visibilityToggle.click();
    await page.waitForTimeout(500);

    const isVisible = await visibilityToggle.getAttribute('aria-pressed') === 'true' ||
  ...
  test('renomme un calque', async ({ page }) => {
    ...
    const firstLayer = layersPanel.locator('.layer-item, [data-testid="layer-item"]').first();
    const layerName = firstLayer.locator('.layer-name, [data-testid="layer-name"]').first();
  ...
  test('supprime un calque', async ({ page }) => {
    ...
    const deleteButton = firstLayer.locator('[data-testid="layer-delete"], button[aria-label*="delete"], button[aria-label*="supprimer"]').first();
...
test.describe('Projections cartographiques - 2.B.4.c', () => {
  ...
  test('attribue une projection par défaut', async ({ page }) => {
    const projectionTool = page.locator('[data-testid="projection-tool"]').or(page.locator('.projection-tool'));
    await expect(projectionTool).toBeVisible({ timeout: 10000 });

    const currentProjection = projectionTool.locator('.current-projection, [data-testid="current-projection"]').first();
  ...
  test('filtre par catégorie de projection', async ({ page }) => {
    ...
    const categoryFilter = projectionTool.locator('[data-testid="projection-category"], select, .category-filter').first();
    await categoryFilter.selectOption({ index: 1 });

    await page.waitForTimeout(1000);

    const projectionList = projectionTool.locator('.projection-item, [data-testid="projection-item"]');
  ...
  test.skip('personnalise les paramètres de projection', async ({ page }) => {});

  test('applique les changements en temps réel', async ({ page }) => {
    const projectionTool = page.locator('[data-testid="projection-tool"]').or(page.locator('.projection-tool'));
    await expect(projectionTool).toBeVisible({ timeout: 10000 });

    const firstProjection = projectionTool.locator('.projection-item, [data-testid="projection-item"]').first();
    await firstProjection.click();

    await page.waitForTimeout(1000);

    const map = page.locator('.map-container, #map, [data-testid="map"]').first();
  ...
  test.skip('active l\'aperçu simplifié pour performance', async ({ page }) => {});
});

test.describe('Simplification - 2.B.4.d', () => {
  test.skip('simplifie un fond de carte avec différents niveaux', async ({ page }) => {});

  test.skip('avertit du risque de suppression', async ({ page }) => {});

  test.skip('applique à toutes les cartes en collection', async ({ page }) => {});
});

test.describe('Collections de cartes - 2.B.4.e', () => {
  test.skip('crée une collection depuis variables multiples', async ({ page }) => {});

  test.skip('sélectionne plusieurs variables pour la collection', async ({ page }) => {});

  test.skip('gère les échelles commune et propre', async ({ page }) => {});

  test.skip('configure la disposition des cartes', async ({ page }) => {});

  test.skip('synchronise les paramètres de projection entre cartes', async ({ page }) => {});

  test.skip('synchronise le niveau de zoom entre cartes', async ({ page }) => {});

  test.skip('applique le même fond de carte à toute la collection', async ({ page }) => {});
});