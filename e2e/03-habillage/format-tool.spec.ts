...
test.describe('Outil Format - Habillage', () => {
  test('ouvre l\'outil de format', async ({ page }) => {
    ...
    const formatTool = page.locator('button:has-text("Format"), [aria-label*="Format"]').first();
    ...
    if (await formatTool.count() > 0) {
      ...
      const formatPanel = page.locator('#khartis-format-tool, [class*="format"]').first();
  ...
  test('affiche les modes prédéfini et personnalisé', async ({ page }) => {
    ...
    if (await formatTool.count() > 0) {
      ...
      const presetTab = page.locator('button:has-text("Prédéfini"), [role="tab"]:has-text("Preset")').first();
      const customTab = page.locator('button:has-text("Personnalisé"), [role="tab"]:has-text("Custom")').first();
  ...
  test('permet de changer la couleur de fond', async ({ page }) => {
    ...
    if (await formatTool.count() > 0) {
      ...
      const colorSelector = page.locator('[class*="color-selector"], button:has-text("Couleur")').first();
  ...
  test('permet de régler les marges', async ({ page }) => {
    ...
    if (await formatTool.count() > 0) {
      ...
      const marginInputs = page.locator('[class*="margin"] input[type="number"]');
  ...
  test('affiche et active la grille d\'alignement', async ({ page }) => {
    ...
    if (await formatTool.count() > 0) {
      ...
      const gridToggle = page.locator('[class*="grid-toggle"], .bx--toggle:has-text("Grille")').first();
      ...
      if (await gridToggle.count() > 0) {
        ...
        const gridOverlay = page.locator('[class*="grid-overlay"], [class*="grid-lines"]').first();
