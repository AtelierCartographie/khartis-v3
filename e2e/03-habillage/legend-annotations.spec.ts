...
test.describe('Outil Légende', () => {
  test('ouvre l\'outil de légende', async ({ page }) => {
    ...
    const legendTool = page.locator('button:has-text("Légende"), [aria-label*="Légende"]').first();
    ...
    if (await legendTool.count() > 0) {
      ...
      const legendPanel = page.locator('[class*="legend"], #khartis-legend-tool').first();
  ...
  test('affiche/masque la légende', async ({ page }) => {
    ...
    if (await legendTool.count() > 0) {
      ...
      const visibilityToggle = page.locator('button[aria-label*="visibility"], .bx--toggle').first();
  ...
  test('permet de modifier la police', async ({ page }) => {
    ...
    if (await legendTool.count() > 0) {
      ...
      const fontSelect = page.locator('select:has(option:has-text("Arial")), [role="combobox"]').first();
  ...
  test('permet de régler la taille du texte', async ({ page }) => {
    ...
    if (await legendTool.count() > 0) {
      ...
      const sizeControl = page.locator('.bx--slider, input[type="number"][max*="2"], input[type="range"]').first();
  ...
  test('permet d\'ajouter un arrière-plan', async ({ page }) => {
    ...
    if (await legendTool.count() > 0) {
      ...
      const bgToggle = page.locator('.bx--toggle:has-text("Arrière-plan"), .bx--toggle:has-text("Background")').first();
      ...
      if (await bgToggle.count() > 0) {
        ...
        const opacitySlider = page.locator('.bx--slider:has-text("Opacité"), .bx--slider:has-text("Opacity")').first();
...
test.describe('Annotations - Texte', () => {
  test('ouvre l\'outil d\'annotations texte', async ({ page }) => {
    ...
    const annotationsTool = page.locator('button:has-text("Annotations"), button:has-text("Texte")').first();
    ...
    if (await annotationsTool.count() > 0) {
      ...
      const annotationsPanel = page.locator('[class*="annotations"], [class*="text-tool"]').first();
  ...
  test('ajoute une zone de texte', async ({ page }) => {
    ...
    if (await annotationsTool.count() > 0) {
      ...
      const addButton = page.locator('button:has(svg[class*="add"]), button:has-text("Ajouter")').first();
  ...
  test('permet de choisir un style prédéfini', async ({ page }) => {
    ...
    if (await annotationsTool.count() > 0) {
      ...
      const styleSelect = page.locator('select:has(option:has-text("Note")), select:has(option:has-text("Titre"))').first();
  ...
  test('permet de modifier la taille du texte', async ({ page }) => {
    ...
    if (await annotationsTool.count() > 0) {
      ...
      const sizeInput = page.locator('input[type="number"][min], .bx--number input').first();
  ...
  test('permet de changer l\'alignement du texte', async ({ page }) => {
    ...
    if (await annotationsTool.count() > 0) {
      ...
      const alignButtons = page.locator('button[aria-label*="align"], button:has(svg[class*="text-align"])');
