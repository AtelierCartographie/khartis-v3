...
async function createTestProject(
  page: any,
  projectName: string = 'Test Habillage'
) {
  await page.goto('/');
...
test.describe('Outil Format - 2.C.2.a', () => {
  test('configure le format et la mise en page', async ({ page }) => {
    ...
    const formatTool = page
      .locator('[data-testid="format-tool"]')
      .or(page.locator('button:has-text("Format")'))
      .first();
    if ((await formatTool.count()) > 0) {
      ...
      const formatOptions = page
        .locator('[data-testid="format-option"]')
        .or(page.locator('.format-option'));
      const count = await formatOptions.count();
      expect(count).toBeGreaterThan(0);
    }
  });

  test.skip("gère la grille d'alignement avec magnétisme", async ({
    page
  }) => {});
});
...
test.describe('Outil Légende - 2.C.2.b', () => {
  test('édite et personnalise les légendes', async ({ page }) => {
    ...
    const legendTool = page
      ...
      .or(page.locator('button:has-text("Légende")'))
      .first();
    if ((await legendTool.count()) > 0) {
      await legendTool.click();
      await page.waitForTimeout(500);

      const legendEditor = page
        .locator('[data-testid="legend-editor"]')
        .or(page.locator('.legend-editor'));
      await expect(legendEditor).toBeVisible({ timeout: 5000 });
    }
  });

  test.skip('applique les styles à toutes les légendes', async ({
    page
  }) => {});
});
...
test.describe('Indications géographiques - 2.C.2.c', () => {
  test('ajoute et configure une échelle', async ({ page }) => {
    ...
    const scaleTool = page
      .locator('[data-testid="scale-tool"]')
      .or(page.locator('button:has-text("Echelle")'))
      .first();
    if ((await scaleTool.count()) > 0) {
      await scaleTool.click();
      await page.waitForTimeout(500);

      const scaleElement = page
        .locator('[data-testid="scale-element"]')
        .or(page.locator('.scale-bar'));
...
test.describe('Annotations - Texte - 2.C.2.d', () => {
  test('ajoute une zone de texte', async ({ page }) => {
    ...
    const textTool = page
      .locator('[data-testid="text-tool"]')
      .or(page.locator('button:has-text("Texte")'))
      .first();
    if ((await textTool.count()) > 0) {
      ...
      const canvas = page
        .locator('.map-canvas, #map, [data-testid="map"]')
        .first();
  ...
  test('définit le contenu du texte', async ({ page }) => {
    ...
    if ((await textTool.count()) > 0) {
      ...
      const textInput = page
        ...
        .or(page.locator('input[type="text"], textarea'))
        .first();
...
test.describe('Annotations - Formes - 2.C.2.d', () => {
  test.skip('ajoute et configure différentes formes géométriques', async ({
    page
  }) => {});

  test.skip('modifie et supprime des formes existantes', async ({
    page
  }) => {});
});
...
test.describe('Annotations - Dessin - 2.C.2.d', () => {
  ...
  test.skip('applique un lissage automatique au dessin', async ({
    page
  }) => {});
  ...
  test.skip('dessine avec pression variable (si supporté)', async ({
    page
  }) => {});
...
test.describe('Annotations - Image - 2.C.2.d', () => {
  ...
  test.skip("modifie les réglages d'une image existante", async ({
    page
  }) => {});
...
test.describe('Déficiences visuelles - 2.C.2.e', () => {
  test.skip('simule la protanopie (absence de rouge - 1% hommes)', async ({
    page
  }) => {});

  test.skip('simule la deutéranopie (absence de vert - 1% hommes)', async ({
    page
  }) => {});

  test.skip('simule la tritanopie (absence de bleu - 0.001% population)', async ({
    page
  }) => {});

  test.skip("simule l'achromatopsie (absence totale de couleur)", async ({
    page
  }) => {});

  test.skip('bascule entre différents filtres rapidement', async ({
    page
  }) => {});

  test.skip("n'affecte pas l'export de la carte", async ({ page }) => {});

  test.skip("conserve les couleurs originales pour l'export", async ({
    page
  }) => {});
});
