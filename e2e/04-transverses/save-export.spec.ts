...
test.describe('Sauvegarde automatique', () => {
  ...
  test('affiche le statut de sauvegarde', async ({ page }) => {
    ...
    const saveIndicator = page.locator('text=/sauv|saved|enregistr/i, [aria-label*="save"]').first();
  ...
  test('permet de sauvegarder manuellement', async ({ page }) => {
    ...
    const saveButton = page.locator('button[aria-label*="save"], button:has(svg[class*="save"])').first();
...
test.describe('Export du projet', () => {
  test('affiche le bouton de téléchargement', async ({ page }) => {
    ...
    const downloadButton = page.locator('button[aria-label*="download"], button[aria-label*="télécharger"], button:has(svg[class*="download"])').first();
  ...
  test('ouvre le menu d\'export', async ({ page }) => {
    ...
    if (await downloadButton.count() > 0) {
      ...
      const exportMenu = page.locator('[class*="export"], [class*="download"], .bx--overflow-menu--open').first();
  ...
  test('propose plusieurs formats d\'export', async ({ page }) => {
    ...
    if (await downloadButton.count() > 0) {
      ...
      const exportOptions = page.locator('[role="menuitem"], button:has-text(".kh"), button:has-text("PNG"), button:has-text("SVG")');
  ...
  test('permet d\'exporter en .kh', async ({ page }) => {
    ...
    if (await downloadButton.count() > 0) {
      ...
      const khOption = page.locator('[role="menuitem"]:has-text(".kh"), button:has-text("Projet")').first();
...
test.describe('Gestion des versions', () => {
  test('conserve l\'historique des modifications', async ({ page }) => {
    ...
    const undoButton = page.locator('button[aria-label*="undo"], button[aria-label*="annuler"]').first();
