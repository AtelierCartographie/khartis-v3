...
export const isDbInitializing = derived(duckDbState, ($state) => $state.isInitializing);
...
class DuckDBManager {
	...
	async initialize(): Promise<void> {
		...
		try {
			duckDbState.update((state) => ({ ...state, isInitializing: true, error: null }));
			...
			const errorMessage = error instanceof Error ? error.message : 'Failed to initialize DuckDB';
	...
	async registerFile(name: string, url: string, protocol = 4, cache = false): Promise<void> {
		const currentState = get(duckDbState);
		if (!currentState.db) {
	...
	async query<T = Record<string, unknown>>(sql: string): Promise<T[]> {
		...
		try {
			queryState.update((state) => ({ ...state, isLoading: true, error: null }));
			...
			queryState.update((state) => ({ ...state, isLoading: false, error: null }));
			return rows;
		} catch (error) {
			const errorMessage = error instanceof Error ? error.message : 'Query failed';
			console.error('Query failed:', error);

			queryState.update((state) => ({ ...state, isLoading: false, error: errorMessage }));
			throw error;
		}
	}

	async createTableFromFile(tableName: string, fileName: string): Promise<void> {
