...
export class DuckDBValidatorService {
	static async validateWithDuckDB(
		...
		try {
			logger.info('Starting DuckDB validation', LogCategory.DUCKDB, { tableName });
			...
			let suggestedCatalogue: { catalogueId: string; confidence: number } | undefined;

			if (dataAnalysis.geoDetection.hasGeoColumns && dataAnalysis.geoDetection.suggestedPrimaryGeoColumn) {
				...
				if (options.catalogueId) {
					...
					suggestedCatalogue = await GeoMatcher.suggestCatalogue(
						geoValues.slice(0, 100)
					) || undefined;
	...
	private static async installAnalysisMacros(): Promise<void> {
		...
		try {
			...
			logger.error('Failed to install analysis macros', LogCategory.DUCKDB, error);
			throw error;
		}
	}
		...
		try {
			...
			for (const header of headers) {
				try {
					...
					logger.warn(`Failed to analyze column ${header}`, LogCategory.DUCKDB, error);
				}
			}
	...
	static formatValidationReport(validation: ValidationResult): string {
		...
		if (validation.duckdbAnalysis) {
			lines.push('\n=== ANALYSE DUCKDB ===');
			lines.push(`Colonnes analysées: ${validation.duckdbAnalysis.columns.length}`);
			lines.push(`Résumés statistiques: ${validation.duckdbAnalysis.summaries.length}`);
			lines.push(`Histogrammes générés: ${validation.duckdbAnalysis.histograms.length}`);
