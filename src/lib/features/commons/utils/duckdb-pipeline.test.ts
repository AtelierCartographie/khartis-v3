...
// Check if native DuckDB binding exists before trying to import
// The native module requires compilation and may not be available in all environments
const DUCKDB_BINDING_PATH = join(
  process.cwd(),
  'node_modules/.pnpm/duckdb@1.4.3_encoding@0.1.13/node_modules/duckdb/lib/binding/duckdb.node'
);
const nativeModuleAvailable = existsSync(DUCKDB_BINDING_PATH);

// Only import if native module is available (prevents crash on missing .node file)
...
if (nativeModuleAvailable) {
  Database = (await import('duckdb')).default;
}

describe.skipIf(!nativeModuleAvailable)(
  'DuckDB Pipeline Integration Tests',
  () => {
    beforeAll(async () => {
      if (!Database) return;
      ...
      await new Promise<void>((resolve, reject) => {
        conn!.run('INSTALL spatial; LOAD spatial;', (err: Error | null) => {
          if (err) reject(err);
          else resolve();
        });
      });
    ...
    describe('Data Analysis Functions', () => {
      ...
      `);
        ...
        const popCol = columnTypes.find(
          (c) => c.column_name === 'POP_TOT_2023'
        );
