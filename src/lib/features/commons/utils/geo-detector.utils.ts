export interface GeoColumnResult {
	index: number;
	columnName: string;
	type: 'latitude' | 'longitude' | 'country_name' | 'iso2' | 'iso3' | 'region' | 'city' | 'coordinates' | 'unknown';
	confidence: number;
	sampleValues?: string[];
...
export class GeoColumnDetector {
	private static readonly COLUMN_NAME_PATTERNS = {
		...
		region: /^(region|province|department|departement|county|oblast|prefecture)$/i,
	...
	private static readonly COUNTRY_SAMPLES = [
		'FRANCE', 'GERMANY', 'SPAIN', 'ITALY', 'UNITED KINGDOM', 'POLAND',
		'ALLEMAGNE', 'ESPAGNE', 'ITALIE', 'ROYAUME-UNI', 'POLOGNE',
		'DEUTSCHLAND', 'SPANIEN', 'ITALIEN', 'POLEN'
	];

	private static readonly REGION_SAMPLES = [
		'ILE-DE-FRANCE', 'BAVARIA', 'CATALONIA', 'LOMBARDY', 'MAZOWIECKIE',
		'BRETAGNE', 'BAYERN', 'ANDALUSIA', 'LAZIO', 'WIELKOPOLSKIE'
	];

	private static readonly CITY_SAMPLES = [
		'PARIS', 'BERLIN', 'MADRID', 'ROME', 'WARSAW', 'LONDON',
		'LYON', 'MUNICH', 'BARCELONA', 'MILAN', 'KRAKOW', 'MANCHESTER'
	];
	...
	private static detectByValues(
		...
		const iso2Match = stringValues.filter(v => this.VALUE_PATTERNS.iso2(v)).length / stringValues.length;
		if (iso2Match > 0.8) {
			return {
				type: 'iso2',
		...
		const iso3Match = stringValues.filter(v => this.VALUE_PATTERNS.iso3(v)).length / stringValues.length;
		if (iso3Match > 0.8) {
			return {
				type: 'iso3',
		...
		const latMatch = stringValues.filter(v => this.VALUE_PATTERNS.latitude(v)).length / stringValues.length;
		if (latMatch > 0.8) {
			return {
				type: 'latitude',
				confidence: latMatch,
		...
		const lonMatch = stringValues.filter(v => this.VALUE_PATTERNS.longitude(v)).length / stringValues.length;
		if (lonMatch > 0.8) {
			return {
				type: 'longitude',
				confidence: lonMatch,
		...
		const countryMatch = this.matchAgainstSamples(stringValues, this.COUNTRY_SAMPLES);
		if (countryMatch > 0.3) {
			return {
				type: 'country_name',
		...
		const regionMatch = this.matchAgainstSamples(stringValues, this.REGION_SAMPLES);
		if (regionMatch > 0.3) {
			return {
				type: 'region',
	...
	private static validateColumnValues(type: string, values: any[]): number {
		const validator = this.VALUE_PATTERNS[type as keyof typeof this.VALUE_PATTERNS];
	...
	private static matchAgainstSamples(values: string[], samples: string[]): number {
	...
	private static selectPrimaryGeoColumn(columns: GeoColumnResult[]): GeoColumnResult | undefined {
		if (columns.length === 0) return undefined;

		const priorityOrder: GeoColumnResult['type'][] = [
			'country_name', 'iso3', 'iso2', 'region', 'city', 'coordinates', 'latitude'
		];
