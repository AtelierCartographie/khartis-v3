...
export class GeoMatcher {
	...
	static async validateAgainstCatalogue(
		...
		const normalizedValues = this.preprocessValues(values, options.caseSensitive);
		...
		result.catalogueCoverage = (uniqueMatches.size / catalogue.entries.size) * 100;

		result.matchRate = values.length > 0
			? result.matched.length / values.length
			: 0;
	...
	private static normalize(value: string, caseSensitive?: boolean): string {
		...
		for (const [accent, base] of Object.entries(accentsMap)) {
			...
			normalized = normalized.replace(new RegExp(accent.toLowerCase(), 'g'), base.toLowerCase());
		}
	...
	private static calculateConfidence(
		...
		if (result.fuzzyMatches.length > 0) {
			const avgFuzzySimilarity = result.fuzzyMatches.reduce(
	...
	static async loadCatalogue(catalogueId: string): Promise<CatalogueInfo | null> {
		...
		const mockCatalogues: Record<string, CatalogueInfo> = {
			'world_countries': {
				...
				entries: new Set([
					...
					'POLOGNE', 'ROUMANIE', 'PAYS-BAS', 'BELGIQUE', 'GRECE',
					'PORTUGAL', 'TCHEQUIE', 'HONGRIE', 'SUEDE', 'AUTRICHE',
					'BULGARIE', 'DANEMARK', 'FINLANDE', 'SLOVAQUIE', 'IRLANDE',
					'CROATIE', 'LITUANIE', 'SLOVENIE', 'LETTONIE', 'ESTONIE',
					'CHYPRE', 'LUXEMBOURG', 'MALTE'
				]),
				alternativeNames: new Map([
					['ALLEMAGNE', ['GERMANY', 'DEUTSCHLAND', 'ALEMANIA']],
					['ESPAGNE', ['SPAIN', 'ESPAÑA', 'SPANIEN']],
					['ROYAUME-UNI', ['UNITED KINGDOM', 'UK', 'GRANDE-BRETAGNE', 'GREAT BRITAIN']],
			...
			'france_regions': {
				...
				entries: new Set([
					...
					'NORMANDIE', 'BRETAGNE', 'BOURGOGNE-FRANCHE-COMTE',
					'CENTRE-VAL DE LOIRE', 'CORSE', 'GUADELOUPE',
					'MARTINIQUE', 'GUYANE', 'LA REUNION', 'MAYOTTE'
				]),
				alternativeNames: new Map([
					['PROVENCE-ALPES-COTE D\'AZUR', ['PACA', 'PROVENCE-ALPES-CÔTE D\'AZUR']],
	...
	static async suggestCatalogue(
		...
		const cataloguesToTest = availableCatalogues || ['world_countries', 'france_regions'];
		const results: Array<{ catalogueId: string; matchRate: number; confidence: number }> = [];
		...
		for (const catalogueId of cataloguesToTest) {
			try {
				...
				console.error(`Erreur lors du test du catalogue ${catalogueId}:`, error);
			}
		}
	...
	static formatMatchReport(result: MatchResult): string {
		...
		lines.push(`Taux de correspondance: ${(result.matchRate * 100).toFixed(1)}%`);
		lines.push(`Confiance: ${(result.confidence * 100).toFixed(1)}%`);

		if (result.catalogueCoverage !== undefined) {
			lines.push(`Couverture du catalogue: ${result.catalogueCoverage.toFixed(1)}%`);
		}
		...
		if (result.fuzzyMatches.length > 0) {
			...
			result.fuzzyMatches.slice(0, 5).forEach(match => {
				lines.push(`   - "${match.original}" → "${match.suggestion}" (${(match.similarity * 100).toFixed(0)}%)`);
			});
		}
