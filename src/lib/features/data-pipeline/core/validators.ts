...
export async function validateFile(file: File): Promise<ValidationResult> {
	...
	if (file.size === 0) {
		logger.warn('Uploaded file is empty', LogCategory.DATA, { fileName: file.name });
	...
	if (file.size > MAX_FILE_SIZE) {
		...
		return validationFailure([`File size exceeds ${MAX_FILE_SIZE / (1024 * 1024)}MB limit`]);
	}
...
export function validateMimeType(file: File, allowedMimeTypes: string[]): ValidationResult {
