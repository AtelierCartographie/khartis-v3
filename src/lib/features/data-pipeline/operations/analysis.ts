...
export function enrichColumns(columns: DuckAnalyticsColumn[]): EnrichedColumn[] {
...
export async function buildDatasetFromDuckTable(
	...
	const duckdbColumns = (await Duck.analyse(tableName)) as DuckAnalyticsColumn[];
	...
	if (dataset.analysis) {
		dataset.analysis.warnings = [...(dataset.analysis.warnings ?? []), ...qualityWarnings];
	}
...
function buildDatasetResult(params: {
	...
	const { file, tableName, enrichedColumns, rowCount, isGeoFile, geometryInfo, format } = params;
	...
	return {
		...
		analysis: {
			...
			geoColumns: geometryInfo
				? [{ columnName: geometryInfo.type ? 'geom' : '', type: 'unknown' as const, confidence: 1, index: 0, isValid: true }]
				: [],
			rowCount,
			warnings: []
		},
