...
export function fromDuckDBType(duckType: string): ColumnType {
	...
	if (normalized.includes('date') || normalized.includes('time')) return ColumnType.DATE;
	if (
	...
	if (normalized.includes('geometry') || normalized.includes('geom')) return ColumnType.GEOMETRY;
...
export function hasNumericStats(stats: ColumnStats): boolean {
	return stats.mean !== undefined && stats.median !== undefined && stats.stdDev !== undefined;
}
...
export function computeCentroid(bounds: [number, number, number, number]): [number, number] {
...
export function computeBoundsArea(bounds: [number, number, number, number]): number {
...
export function extractPrimaryGeometryType(metadata: GeoArrowMetadata): string | undefined {
...
export interface ProcessedDataset {
	...
	geoDetection?: GeoDetectionResult;
	originalData?: { columns: ColumnInfo[]; data: Record<string, unknown>[]; rowCount: number };
}
