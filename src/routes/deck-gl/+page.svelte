<script lang="ts">
	...
	import { ChartBar, Home, Location, Map as MapIcon } from 'carbon-icons-svelte';
	...
	const CITIES_DATA = [
		{ name: 'Paris', coordinates: [2.3522, 48.8566], population: 2161000, country: 'France' },
		{
			name: 'London',
		...
		{ name: 'Berlin', coordinates: [13.405, 52.52], population: 3669000, country: 'Germany' },
		{ name: 'Madrid', coordinates: [-3.7038, 40.4168], population: 3223000, country: 'Spain' },
		{ name: 'Rome', coordinates: [12.4964, 41.9028], population: 2873000, country: 'Italy' },
		{ name: 'Warsaw', coordinates: [21.0122, 52.2297], population: 1790000, country: 'Poland' },
		{ name: 'Vienna', coordinates: [16.3738, 48.2082], population: 1897000, country: 'Austria' },
		{
			name: 'Amsterdam',
		...
		{ name: 'Budapest', coordinates: [19.0402, 47.4979], population: 1752000, country: 'Hungary' },
		{ name: 'Barcelona', coordinates: [2.1734, 41.3851], population: 1620000, country: 'Spain' }
	];
	...
	function getColorByData(countryName: string): [number, number, number, number] {
		...
		if (value === 0 && mappedName !== countryName) {
			console.log(`Pays mappé non trouvé: ${countryName} -> ${mappedName}`);
		} else if (value === 0) {
			console.log(`Pays non trouvé dans les données: ${countryName}`);
		}
	...
	function updateLayers() {
		console.log('updateLayers appelé avec dataset:', currentDataset);
		...
		if (showCities) {
			layers.push(
				new ScatterplotLayer({
					...
					getFillColor: [218, 30, 40, 230], // Rouge IBM
			...
			<Column lg={16}>
				<div class="header-section">
					<div class="title-section">
						...
						<p class="subtitle">Exploration de données géographiques avec deck.gl et MapLibre</p>
					</div>
					...
					<div class="controls-section">
						<Tile class="controls-tile">
							...
							<div class="action-buttons">
								<Button kind="secondary" size="small" icon={Home} on:click={resetView}>
									Vue Europe
								</Button>
								<Button kind="secondary" size="small" icon={Location} on:click={focusOnFrance}>
									Focus France
								</Button>
							</div>
						</Tile>
					...
					{#if currentDataset === 'population'}
						<div class="legend-items">
							<div class="legend-item">
								<div class="legend-color" style="background: rgb(24, 78, 119);"></div>
								<span>> 60M hab.</span>
							</div>
							<div class="legend-item">
								<div class="legend-color" style="background: rgb(15, 98, 254);"></div>
								<span>30-60M hab.</span>
							</div>
							<div class="legend-item">
								<div class="legend-color" style="background: rgb(69, 137, 148);"></div>
								<span>10-30M hab.</span>
							</div>
							<div class="legend-item">
								<div class="legend-color" style="background: rgb(166, 200, 255);"></div>
								<span>5-10M hab.</span>
							</div>
							<div class="legend-item">
								<div class="legend-color" style="background: rgb(224, 242, 254);"></div>
								<span>&lt; 5M hab.</span>
							</div>
						</div>
					{:else}
						<div class="legend-items">
							<div class="legend-item">
								<div class="legend-color" style="background: rgb(22, 135, 78);"></div>
								<span>> 60k€</span>
							</div>
							<div class="legend-item">
								<div class="legend-color" style="background: rgb(36, 161, 72);"></div>
								<span>40-60k€</span>
							</div>
							<div class="legend-item">
								<div class="legend-color" style="background: rgb(66, 190, 101);"></div>
								<span>25-40k€</span>
							</div>
							<div class="legend-item">
								<div class="legend-color" style="background: rgb(163, 240, 163);"></div>
								<span>15-25k€</span>
							</div>
							<div class="legend-item">
								<div class="legend-color" style="background: rgb(221, 243, 221);"></div>
								<span>&lt; 15k€</span>
							</div>
						</div>
					{/if}
